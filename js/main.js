...
function Model() {
	var addTask = function(s) {
		var taskItem = [localStorage['todo']];
		taskItem.push(s);
		localStorage.setItem('todo', taskItem);
		render.init();
	};
	var completeTask = function(t) {
		var arr = [localStorage['completed']];
		arr.push(t);
		localStorage.setItem('completed', arr);
		var newArr = localStorage['completed'].split(',');
...
//换肤功能 主题颜色控制器 
function appColor() {
	var mainColor = localStorage.color || '#25b99a',
		secColor = '#FFC107';
...
//"换肤"面板
function colorPanel() {
	var html = '<ul class=setColorPanel><li class=colorItem data=#25b99a><span>文艺</span><div class=colorBlock><div class=cgreen></div><div class=cgwhite></div></div><li class=colorItem data=#F44336><span>狂热</span><div class=colorBlock><div class=cred></div><div class=crwhite></div></div><li class=colorItem data=#2196F3><span>高冷</span><div class=colorBlock><div class=cblue></div><div class=cbwhite></div></div></ul>';
	$('.menu-subCard').html(html);
	$('.colorItem').click(function(event) {
		mainColor = $(this).attr('data');
		//颜色 本地存储
		localStorage.color = mainColor;
		appColor();
		//动效
...
//"关于"面板
function aboutPanel() {
	var html = '<div class=aboutApp><p><b>清单，一个专治拖延症患者的应用。</b><br>当前为测试版本，持续更新中...<br><br>@2016 <a href="http://liaokeyu.com/about.html" target="_blank">廖柯宇</a></div>';
	$('.menu-subCard').html(html);
}
